...
def deleteStu():
    ...
    while (True):
        readStudent()
        showStuInfo()
        inp = input("Delete student where student no > ")
        arr = inp.split()
        with open(path.url_student, "r", encoding="UTF-8") as file_stu:
            read = file_stu.readlines()
        stu: Student
        ...
        for sno in arr:
            if (checkIn(sno, stuno, target="student no", mode="in")):
                stu = student[sno]
                index = stu.line - 1
                removelist.append(read[index])
                stuno.remove(sno)
                removesno.append(sno)
        print("SNO deletion list:", removesno)
        con = input("Confirm delete?  Yes:1 No:2 > ")
        if (con == "1"):
